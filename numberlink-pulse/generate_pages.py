"""Generate static, full-text policy pages in the repository's supported languages."""
import json
...
ROOT='https://oh-seungjin.github.io/privacy-terms/numberlink-pulse/'
...
for lang,label in LANGS:
    ...
    for kind in KINDS:
        ...
        if kind=='index':
            body=f'<p class="eyebrow">NUMBERLINK PULSE</p><h1>{esc(d["tag"])}</h1><p class="lead">{esc(d["intro"])}</p><div class="cards">'+''.join(f'<a class="card" href="{filename(k,lang)}"><strong>{esc(d["nav"][i])}</strong><span aria-hidden="true">↗</span></a>' for i,k in enumerate(KINDS) if i)+ '</div>'
        else:
        ...
        if kind=='privacy':
            body+='<aside><p><a href="https://www.apple.com/legal/privacy/">Apple Privacy</a> · <a href="https://policies.google.com/privacy">Google Privacy</a> · <a href="https://docs.github.com/en/site-policy/privacy-policies/github-general-privacy-statement">GitHub Privacy</a> · <a href="https://supabase.com/privacy">Supabase</a></p></aside>'
        if kind=='terms':body+='<p><a href="https://www.apple.com/legal/internet-services/itunes/dev/stdeula/">Apple Standard EULA</a></p>'
        subject='NumberLink%20Pulse%20Data%20Deletion' if kind=='delete-account' else 'NumberLink%20Pulse%20Support'
...
<meta name="description" content="NumberLink Pulse · {esc(title)} · Next Studio">
<title>NumberLink Pulse · {esc(title)}</title>
...
<main><div class="top"><a class="brand" href="{filename('index',lang)}"><img src="icon.png" width="40" height="40" alt=""> NUMBERLINK PULSE</a><label><span class="sr-only">Language</span><select class="language" aria-label="Language" onchange="location.href=this.value">{options}</select></label></div>
...
print(f'Generated {len(LANGS)*len(KINDS)} static pages in {len(LANGS)} languages.')
