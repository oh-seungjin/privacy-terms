"""Generate static, full-text policy pages for Neon Block Pulse (ko/en)."""
import json
...
ROOT='https://oh-seungjin.github.io/privacy-terms/neon-block-pulse/'
...
for lang,label in LANGS:
    ...
    for kind in KINDS:
        ...
        if kind=='index':
            body=f'<p class="eyebrow">NEON BLOCK PULSE</p><h1>{esc(d["tag"])}</h1><p class="lead">{esc(d["intro"])}</p><div class="cards">'+''.join(f'<a class="card" href="{filename(k,lang)}"><strong>{esc(d["nav"][i])}</strong><span aria-hidden="true">↗</span></a>' for i,k in enumerate(KINDS) if i)+ '</div>'
        else:
        ...
        subject='Neon%20Block%20Pulse%20Data%20Deletion' if kind=='delete-account' else 'Neon%20Block%20Pulse%20Support'
...
<meta name="description" content="Neon Block Pulse · {esc(title)} · Next Studio">
<title>Neon Block Pulse · {esc(title)}</title>
...
<main><div class="top"><a class="brand" href="{filename('index',lang)}"><img src="icon.png" width="40" height="40" alt=""> NEON BLOCK PULSE</a><label><span class="sr-only">Language</span><select class="language" aria-label="Language" onchange="location.href=this.value">{options}</select></label></div>
...
print('Generated', len(LANGS)*len(KINDS), 'static pages.')
