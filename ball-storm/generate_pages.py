"""Generate static, full-text policy pages for Ball Storm (ko/en)."""
import json
...
ROOT='https://oh-seungjin.github.io/privacy-terms/ball-storm/'
...
for lang,label in LANGS:
    ...
    for kind in KINDS:
        ...
        if kind=='index':
            body=f'<p class="eyebrow">BALL STORM</p><h1>{esc(d["tag"])}</h1><p class="lead">{esc(d["intro"])}</p><div class="cards">'+''.join(f'<a class="card" href="{filename(k,lang)}"><strong>{esc(d["nav"][i])}</strong><span aria-hidden="true">↗</span></a>' for i,k in enumerate(KINDS) if i)+ '</div>'
        else:
        ...
        subject='Ball%20Storm%20Data%20Deletion' if kind=='delete-account' else 'Ball%20Storm%20Support'
...
<meta name="description" content="Ball Storm · {esc(title)} · Next Studio">
<title>Ball Storm · {esc(title)}</title>
...
<main><div class="top"><a class="brand" href="{filename('index',lang)}"><img src="icon.png" width="40" height="40" alt=""> BALL STORM</a><label><span class="sr-only">Language</span><select class="language" aria-label="Language" onchange="location.href=this.value">{options}</select></label></div>
